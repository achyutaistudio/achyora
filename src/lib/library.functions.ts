...
export const getLibraryLimits = createServerFn({ method: "GET" }).handler(async () => ({
...
export const prepareLibraryUpload = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({
      ...
      if (data.size > max)
        return fail("FILE_TOO_LARGE", `Keep files under ${Math.floor(max / (1024 * 1024))} MB.`);
...
export const registerLibraryItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<AchyoraResult<{ item: LibraryItem }>> => {
    ...
    try {
      ...
      console.error("library metadata read failed", err instanceof Error ? err.message : err);
      return fail("UNKNOWN", "That upload could not be verified.");
    }
    if (!facts) return fail("NOT_FOUND", "That upload was not found in storage.");
    ...
    if (!isAllowedMime(facts.mimeType)) {
      await removeStorageObject(path);
      return fail("INVALID_INPUT", "That file type isn't allowed in the Library.");
    }
...
export const openLibraryItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<AchyoraResult<{ url: string }>> => {
...
export const deleteLibraryItem = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { id: string }) => input)
  .handler(async ({ data, context }): Promise<AchyoraResult<{ deleted: true }>> => {
    const { data: row } = await context.supabase
      .from("library_items")
